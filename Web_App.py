...
if unweighted == False and marketcap_weighted == False and volume_sold == False and dataset_analysis == False and gini_coef == False and benchmark == False:
    ...
    st.write("Since 2021, NFTs have experienced tremendous hype in society. To gain a better understanding of the NFT market and its pricing this paper examines the NFT market characteristics using the Rare Pepes collection. For this purpose, one of the largest NFT data sets of a single collection is gathered and made available using an open-source web tool. Based on this, a price index is constructed using the hedonic regression methodology and the performance of the Rare Pepes collection compared to other assets is evaluated. The results of the analysis show that the Rare Pepes collection has generated above-average returns since release, outperforming the compared indices, while maintaining a relatively low correlation. The above-average returns come with higher risk and volatility. Using fixed effects regression, driving factors for the Rare Pepe price formation were identified. The evidence from this regression suggests that supply, distribution, quantity, and aesthetic attributes of a Rare Pepe influence the price formation. By contrast, burns do not have a significant effect on the price, which is especially important for investors. The magnitude of the effects suggests that price formation is currently still inefficient. The results of this paper serve as a window to an understanding of the NFT market and, together with the web tool, provide an important foundation for further research.")
    st.markdown("<hr/>", unsafe_allow_html=True)
    ...
    st.write("Telegram group for traders and collectors: [https://telegram.me/pepetraders](https://telegram.me/pepetraders)")
    ...
    # Freeze tool for the evaluation
    current_month = load_month
